...
### Тестирование и метрики (Benchmark)

Для проверки качества работы анализатора (точность и полнота) используется скрипт бенчмарка. Он автоматически прогоняет анализатор по набору уязвимых (True Positive) и безопасных (True Negative) примеров кода.

Чтобы запустить тестирование и получить отчет по метрикам (TP, FP, TN, FN, Precision, Recall):

```bash
python3 scripts/test_metrics.py
```

Результатом выполнения будет таблица в формате Markdown с детальной статистикой по каждому классу CWE.
